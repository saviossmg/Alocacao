...
function salvarRegistro() {
    ...
    if (check) {
        ...
        $.post(dir + "inserir.php", {
            ...
            if(retorno.status){
                // close the popup
                $("#tipoclasseequipamento_modal").modal("hide");
...
function listar() {
    ...
    $.post(dir + "listar.php", {
        identidade: 3,
        tipo: 'classeequipamentos'
    }, function (data) {
        $(".records_content_tiposclasseequipamentos").html(data);
        $('#tiposclasseequipamentos').dataTable({
    ...
    $.post(dir + "detalhar.php", {
        ...
        function (data, status) {
            ...
            if(retorno.status){
                ...
                // abre o modal
                $("#tipoclasseequipamento_modal").modal("show");
            }
            else{
                bootbox.alert(retorno.mensagem);
            }
...
$(document).ready(function () {
    ...
    $('#tiposclasseequipamentos').dataTable();
    $('#tipoclasseequipamento_modal').on('hide.bs.modal', function (event) {
        $('#tipo_classeequipamento').bootstrapValidator("resetForm",true);
        $('#tipo_classeequipamento').each(function () {
            this.reset();
            $("#id").val("");
        })
    });
...
function validar() {
    ...
    $('#tipo_classeequipamento').bootstrapValidator({
    ...
    return $('#tipo_classeequipamento').data('bootstrapValidator').isValid();
}