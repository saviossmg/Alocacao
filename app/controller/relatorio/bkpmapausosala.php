...
try {
    ...
    $salasImpressao = getSalalab($entityManager,$campus->getId(),1);
    $labsImpressao = getSalalab($entityManager,$campus->getId(),2);

    //dados para serem normatizados e jogados p/ impressão
    $salas = getSalalabOferta($entityManager,$semestre->getId(),$campus->getId(),1);
    $labs = getSalalabOferta($entityManager,$semestre->getId(),$campus->getId(),2);
    $observacoes = getObservacao($entityManager,$semestre->getId(),$campus->getId());

    //gera o html
    if(count($salas) > 0 || count($labs) > 0)
        $htmlx = gerarHtml($salas,$labs,$salasImpressao,$labsImpressao,$observacoes,$semestre,$campus,$entityManager);

    $mpdf->WriteHTML($htmlx);

    $arquivo .= "MAPA_USO_SALAS.pdf";
 ...
 * @return string html
 */
function gerarHtml($salas,$laboratorios,$salasCampus,$labsCampus,$observacoes,$semestre,$campus,$eM){
    //nome de semestre
    ...
    <div style="border: 5; border-style: double;width:1122px; height: 793px;">
        ...
        <!--titulo-->
        <div id="is_titulo">
            <p class="titulo"><b>MAPA DE USO DAS SALAS – '.$nomesem.'</b></p>
        </div>
        <!--titulo-->
        <!--conteudo-->        
        <div id="is_conteudo">
                ...
                <tbody>
                    ...
                    <tr>
                        <td width="140px" height="32px" bgcolor="#c0c0c0">
                            <p class="texto1"><b><center>SALA/BLOCO</p>
                        </td>
 ...
 * @return objeto salas
 */
function getSalalab($eM,$campus,$tipo){
    //seleciona as salas
 ...
 * @return objeto salas
 */
function getSalalabOferta($eM,$semestre,$campus,$tipo){
    //seleciona as salas
    $qb = $eM->createQueryBuilder();
    $qb->select("Als, Sa, O, Sl, S, C, U")
 ...
 * @return objeto obs
 */
function getObservacao($eM,$semestre,$campus){
    //seleciona as salas
    $qb = $eM->createQueryBuilder();
